...
public class Communicator extends Thread {
	...
	public void run() {
		System.out.println("Lancement du thread : " + app.getServer() + ":"
				+ app.getPort());
		try {
			/*
			 ...
			 * System.out.println("ssh"); String [] tab =
			 * session.getPortForwardingL(); for(int i=0; i<tab.length;i++){
			 * System.out.println(tab[i]); } int p =
			...
			if (soc.isConnected()) {
				...
				if ((mess = in.readLine()).equals(String.valueOf(GO))) {
					...
					System.out
							.print("Tentative de verrouillage de la communication : ");
					if (this.BlockVerrou()) {
						...
						// Envoi des infos OK
						String ssid = "XX";
						System.out.println(soc.getLocalAddress() + ":" + portR
								+ ":" + ssid);
						out.println(soc.getLocalAddress() + ":" + portR + ":"
								+ ssid);
						out.flush();
						...
						if (rep != null && rep.length == 3) {
							...
							if (rep[2].equals(ssid) && !rep[2].equals("null")
								...
								cRec = cRec.replace("myport",
										String.valueOf(portR));
								cRec = cRec.replace("myamp",
										String.valueOf(amp));
								...
								pVol.waitFor();
								cVol = this.volumeSet;
								VOLUME = (Math.abs(this.volMax) + Math
										.abs(this.volMin)) / 2;
								VOLUME = this.volMin + VOLUME;
								cVol = cVol.replace("myvol",
										String.valueOf(VOLUME));
								System.out
										.println("Reglage du volume de sortie : "
												+ cVol);
							...
							while (loop) {
								...
								case OPENDOOR:
									...
									for (int i = 0; i < tabS.length; i++) {
										...
										opDoor = Runtime.getRuntime().exec(
												tabS[i]);
								...
								case TS:
									...
									testSound = testSound.replace("$A",
											String.valueOf(this.amp));
									opDoor = Runtime.getRuntime().exec(
											testSound);
									opDoor.waitFor();
								...
								default:
									System.out
											.println("Reception d'un message inconnu : "
													+ sw);
									break;
								}
							}
		 ...
		 */
		ReleaseVerrou();
		System.out.println("Fin du thread : " + app.getPortSsh() + ":"
				+ app.getServer() + ":" + app.getPort());
	}
